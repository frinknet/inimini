 ...
 *   double ration   = inimini_getdbl(cfg, "mix.amount");
 ...
 *   inimini_setarr(cfg, "core.error_rate", arr, cnt);
...
typedef struct imi_entry {
	...
	char *parent;        /* Implied parent section ("vext"), "" if no section */
...
static inline int __imi_parse(inimini_t *cfg, FILE *f, uint32_t flags) {
	...
	while (fgets(line, sizeof(line), f)) {
		...
		if (*l == ';' || *l == '#') {
			...
			if (strlen(current_comment)) strcat(current_comment, "\n");

			strcat(current_comment, l);

			continue;
		}

		if (*l == '[') {
			const char *start = l + 1;
			...
			if (start && end) {
				...
				imi_entry_t *sec_entry = calloc(1, sizeof(imi_entry_t));
				sec_entry->key = NULL;
				sec_entry->parent = strdup(sec_name);
				sec_entry->comment = current_comment[0] ? strdup(current_comment) : NULL;

				__imi_list_append(cfg, sec_entry);
				strcpy(section, sec_name);

				last_entry = sec_entry;
				current_comment[0] = '\0';
			}
		...
		if (eq) {
			*eq++ = '\0';
			...
			char *key = __imi_trim(l);
			char *val = __imi_trim(eq);
			...
			if (flags & IMI_COMMENT && trailing_com) {
				*trailing_com = '\0';
				val = __imi_trim(val);
				trailing_com = __imi_trim(trailing_com + 1);
				current_comment[0] = '\0';

				strcat(current_comment, trailing_com);
			}
			...
			entry->value = strdup(__imi_expand_env(val));
			entry->parent = __imi_extract_parent(full_key);
			entry->comment = current_comment[0] ? strdup(current_comment) : NULL;
			last_entry = entry;
			current_comment[0] = '\0';
...
static inline int __imi_path(const char *progname, char *buf, size_t size) {
		...
		if (appdata) {
			snprintf(buf, size, "%s\\%s.conf", appdata, progname);
		...
		if (home) {
			snprintf(buf, size, "%s\\.%sconf", home, progname);
		...
		if (pkgdir) {
			snprintf(buf, size, "%s/config/%s.conf", pkgdir, progname);
		...
		if (xdg) {
			snprintf(buf, size, "%s/%s/config", xdg, progname);
...
static inline int inimini_load(inimini_t *cfg, const char *progname, uint32_t flags) {
	...
	#if defined(_WIN32)
		snprintf(path, sizeof(path), "C:/ProgramData/%s/config", progname);
	#else
		snprintf(path, sizeof(path), "/etc/%s/config", progname);
	#endif
	...
	snprintf(path, sizeof(path), "./.%sconf", progname);
...
static inline int inimini_write(const inimini_t *cfg, const char *path, uint32_t flags) {
	...
	for (const imi_entry_t *e = cfg->head; e; e = e->next) {
		...
		if (!prev || (prev_parent && strcmp(current_parent, prev_parent))) {
			if (flags & IMI_GITSTYLE && strchr(current_parent, ' ')) fprintf(f, "[%s \"%s\"]\n", current_parent, current_parent);
			else if (strlen(current_parent)) fprintf(f, "[%s]\n", current_parent);

			printed_sections++;
		}

		if (e->key == NULL) {
			if (e->comment && (flags & IMI_COMMENT)) fprintf(f, "; %s\n", e->comment);
		} else {
			...
			if (flags & IMI_GITSTYLE && prev_parent) fprintf(f, "\t%s = %s", child, e->value);
			else fprintf(f, "%s = %s", child, e->value);

			if (e->comment && (flags & IMI_COMMENT)) fprintf(f, "\n; %s", e->comment);
...
static inline int inimini_merge(inimini_t *base, const inimini_t *overlay, uint32_t flags) {
	...
	while (o) {
		...
		if (b) {
			...
			if ((flags & IMI_COMMENT) && o->comment) {
 ...
 *   - Section list from getsec() (array only; individual strings must be freed per-element)
 *   - Any new allocations explicitly documented above
 *
 * DO NOT FREE:
 *   - Strings from getstr()/getint()/getdbl() (internal, tied to cfg lifetime)
 *   - Array pointer from getarr() (cache part of cfg, internal)
 *   - Entry->comment field (freed automatically with cfg)
 *   - Entry->parent field (freed automatically with cfg)
 *
 * SAFETY: inimini_free() is idempotent. Safe to call on NULL cfg or multiple times.
 */
